...
```{r}
table(cleandata$Churn)/nrow(cleandata)  # Accuracy: 73.42%
```
...
To sum up, if we use a cutoff point of 0.5 and just guess without any statistical models, we will get roughly 73% right. However, we will get approximately 80% right if we use our second logistic regression model. And this increased accuracy will help the Telco company predict whether a customer will churn or not more accurately, and this analysis will be helpful for them when they develop their business strategy to retain their customers. 